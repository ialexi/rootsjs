...
Dobby.Pig.__current_guid = 0; // everyone ++(this)'s, so it is really easy to check if(guid)
...
Dobby.Pig.prototype = {
	...
	*/
	_connect: function(path, hash, target, action) {
	...
	_disconnect: function(path, hash) {
	...
	*/
	connect: function(path, target, action) {
		...
		if (!target.__pig_guid) target.__pig_guid = "guid-" + (++(Dobby.Pig.__pig_guid));
		if (typeof action != "string" && !action.__pig_guid) action.__pig_guid = ++Dobby.Pig.__current_guid;
	
		var hash = target.__pig_guid + "::" + (typeof action == "string" ? action : action.__current_guid);
